...
import 'package:eatoutroundabout/screens/home/show_popups.dart';
...
import 'package:eatoutroundabout/widgets/venue_recommendations_item.dart';
...
class _ShowVenuesState extends State<ShowVenues> {
  ...
  getData() async {
    ...
    for (int i = 0; i < querySnapshot.docs.length; i++) {
      ...
      allVenues.add(venue);
      DocumentSnapshot doc = await voucherService.getPostCodeData(
          allVenues[i].postCode!.replaceAll(" ", "").toLowerCase());
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        RadioListTile(
                          ...
                          contentPadding: EdgeInsets.zero,
                          groupValue: sortBy,
                          title: Text('Food Type', textScaleFactor: 1.1),
                          value: 'foodTypes',
                          onChanged: (val) => setState(() => sortBy = val!),
                        ),
                        RadioListTile(
                        ...
                        RadioListTile(
                          ...
                          groupValue: sortBy,
                          title: Text(
                              'Pre-Theatre Dining', textScaleFactor: 1.1),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        RadioListTile(
                          ...
                          groupValue: filter,
                          title: Text(
                              'Near home (${userController.currentUser.value
                                  .livePostcodeDocId?.toUpperCase()})',
                              textScaleFactor: 1.1),
                          value: 'home',
                          onChanged: (val) => setState(() => filter = val!),
                        ),
                        RadioListTile(
                          ...
                          contentPadding: EdgeInsets.zero,
                          groupValue: filter,
                          title: Text('Work (${userController.currentUser.value
                              .workPostcodeDocId == '' ? 'N/A' : userController
                              .currentUser.value.workPostcodeDocId
                              ?.toUpperCase() ?? 'N/A'})',
                              textScaleFactor: 1.1),
                          value: 'work',
                          onChanged: (val) {
                        ...
                        RadioListTile(
                          ...
                          title: dropdownSearch(
                              'Visit', 'Visit a Place', lauaConstants),
                          value: 'visit',
                          onChanged: (val) =>
                          visit == ''
                              ? Get.defaultDialog(
            ...
            onPressed: () async {
              ...
              await cloudFunction(
                  ...
                  parameters: {
                    ...
                    'workPostcodeDocId': workPostCodeTEC.text.replaceAll(
                        " ", "").toLowerCase(),
                  },
                  action: () async {
      ...
      return MediaQuery(
        ...
        child: DropdownSearch<String>(
          ...
          onChanged: (onChangedElement) async {
            ...
            Get.back();
            utilService.showLoading();
            int i = lauaConstants.indexWhere((element) =>
            element == onChangedElement);
                    ...
                    onTap: () async {
                      if (userController.currentUser.value.accountID != null &&
                        ...
                        if (isAccountApproved)
                          ...
                          utilService.showInfoDialog('Awaiting approval',
                              'Your account is awaiting approval. If you have not heard from us within 48 hours, please contact support@EatOutRoundAbout.co.uk',
                              false);
                      } else
                        addAccount(context, true);
                    },
                    //onTap: () =>Get.to(()=> AddAccount()),
                    child: ColorCard(title: 'Manage a venue?',
                        subTitle: 'Add your venue here, if you have a stunning restaurant or cafe and value sourcing locally. It\'s free to feature on Eat Out Round About and we\'ll fund your voucher redemptions.',
                        color: orangeColor),
                  ),
                  InkWell(
                    onTap: () => Get.to(() => RecommendVenue()),
                    child: ColorCard(title: 'Recommend a Venue',
                        subTitle: 'We’d love to have venues open in your area. Why not recommend your favourites and we’ll contact them? It’s free for venues to list!',
                        color: redColor),
                  ),
                ],
              ),
  ...
  showRecommendationsList() {
    final recommendationID = userController.currentUser.value.recommendationID;
    return PaginateFirestore(
      key: GlobalKey(),
      padding: const EdgeInsets.only(bottom: 25),
      itemBuilderType: PaginateBuilderType.listView,
      itemBuilder: (context, documentSnapshot, i) {
        Venue venue = documentSnapshot[i].data() as Venue;
        return VenueRecommendationsItem(venue: venue, isMyVenue: false);
      },
      query: firestoreService
          .getRecommendationsForUser(recommendationID!),
      ...
      onEmpty: Padding(
        ...
        child: Column(
          children: [
            Text(
              'No venue recommendations to display',
              textScaleFactor: 1.25,
              style: TextStyle(color: Colors.white),
            ),
          ],
