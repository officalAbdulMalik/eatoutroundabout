...
import 'package:eatoutroundabout/screens/admin/account/approve_venues.dart';
...
import 'package:eatoutroundabout/screens/admin/account/orders.dart';
...
import 'package:eatoutroundabout/screens/help/help_home.dart';
...
import 'package:eatoutroundabout/screens/profile/calculate_impact.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    if (showVenueDashboard())
      controller.tabIndex = 2;
    if (showVenueDashboard() && showDashboard())
      ...
      builder: (HomeController controller) {
        return Scaffold(
                ...
                Obx(
                    ...
                    icon: Column(
                      ...
                      children: [
                        ...
                        if (userController.currentUser.value.unreadNotifications == true) Align(alignment: Alignment.bottomCenter, child: CircleAvatar(radius: 5, backgroundColor: redColor)),
                      ],
                    ),
                    onPressed: () async {
                ...
                children: [
                  Expanded(
                    child: ListView(
                      children: [
                        ...
                        // ),
                        if (FirebaseAuth.instance.currentUser!.phoneNumber == '+447786060113' || FirebaseAuth.instance.currentUser!.phoneNumber == '+448805575606') ListTile(leading: Icon(Icons.dialpad, color: Colors.white), title: Text('Approve Venues', textScaleFactor: 1.25, style: TextStyle(color: Colors.white)), onTap: () => goto(ApproveVenues())),
                        // ListTile(leading: Icon(FontAwesomeIcons.list, color: Colors.white), title: Text('Place order', textScaleFactor: 1.25, style: TextStyle(color: Colors.white)), onTap: () => {Get.to(() => CreateOrder())}),
                        Divider(color: Colors.white30, height: padding),
                       ...
                       // Divider(color: Colors.white30, height: padding),
                        ...
                        ListTile(leading: Icon(FontAwesomeIcons.cog, color: Colors.white), title: Text('Update Account', textScaleFactor: 1.25, style: TextStyle(color: Colors.white)), onTap: () => {Get.to(() => EditAccount())}),
                        ListTile(leading: Icon(FontAwesomeIcons.list, color: Colors.white), title: Text('Order History', textScaleFactor: 1.25, style: TextStyle(color: Colors.white)), onTap: () => {Get.to(() => AccountOrders(accountID: userController.currentUser.value.accountID!))}),
                        ListTile(leading: Icon(FontAwesomeIcons.fileInvoice, color: Colors.white), title: Text('Invoices', textScaleFactor: 1.25, style: TextStyle(color: Colors.white)), onTap: () => utilService.showPromoCodeDialog()),
                         if (userController.isBusinessAdmin())
                        Divider(color: Colors.white30, height: padding),
                  ...
                  Row(
                    children: [
                      ...
                      // Expanded(
                      //   child: InkWell(
                      //     onTap: () => utilService.openLink('https://wa.me/message/IC4Q25UGDJGZA1'),
                      //     child: Column(
                      //       children: [
          ...
          bottomNavigationBar: BottomNavigationBar(
            items: [
              if (showDashboard()) BottomNavigationBarItem(label: 'Value', icon: Icon(Icons.dashboard, size: MediaQuery.of(context).size.height * 0.020), activeIcon: Icon(Icons.dashboard)),
              if (showVenueDashboard()) BottomNavigationBarItem(label: 'Venue', icon: Icon(Icons.restaurant_menu, size: MediaQuery.of(context).size.height * 0.020), activeIcon: Icon(Icons.restaurant_menu)),
              BottomNavigationBarItem(label: 'My Impact', icon: Icon(Icons.area_chart, size: MediaQuery.of(context).size.height * 0.020), activeIcon: Icon(Icons.area_chart)),
  ...
  getUserType() {
    if (userController.currentUser.value.businessProfileAdmin!.isNotEmpty)
      return 0;
    if (userController.currentUser.value.venueAdmin!.isNotEmpty!)
      return 0;
    else
      return 3;
  }
    ...
    userPages() {
      return [
        ...
        if (showVenueDashboard()) VenueHome(),
        MyImpact(),
        ShowVenues(),
        MyVouchers(),
        Profile(),
  ...
  showVenueDashboard() {
    if ( userController.currentUser.value.venueAdmin!.length > 0) {
      return true;
    } else
      return false;
  }
  ...
  helpDialog() {
    Get.defaultDialog(
      ...
      title: 'Need Help?',
      contentPadding: const EdgeInsets.all(padding),
      content: Center(child: Text('Search for help and if you can\'t find what you need then WhatsApp us')),
      actions: [
        CustomButton(text: 'Search Help', color: greenColor, icon:Icon(Icons.help_outline_rounded, color: Colors.white), function: () => goto(HelpHome())),
        CustomButton(text: 'WhatsApp', color: primaryColor, icon:Icon(FontAwesomeIcons.whatsapp, color: Colors.white), function: () => utilService.openLink('https://wa.me/message/IC4Q25UGDJGZA1'),
        )],
    );
  }
