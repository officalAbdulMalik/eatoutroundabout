...
class SelfRedeemVoucher extends StatefulWidget {
  final UserVoucher? userVoucher;

  SelfRedeemVoucher({this.userVoucher});

  @override
  _SelfRedeemVoucherState createState() => _SelfRedeemVoucherState();
}

class _SelfRedeemVoucherState extends State<SelfRedeemVoucher> {
                    ...
                    Expanded(
                      child: SingleChildScrollView(
                        child: Column(
                          children: [
                            SizedBox(height: 25),
                            Text('Enter your total bill amount before any discounts', textScaleFactor: 1.00, style: TextStyle( color: Colors.white)),
                            SizedBox(height: 25),
                            CustomTextField(
                              validate: false,
  ...
  Future<bool> redeemTheVoucher(String voucherCode) async {
    ...
    if (userVoucherDocumentSnapshot.docs.isNotEmpty) {
      ...
      //Get PostCode Data
      DocumentSnapshot postCodeDocumentSnapshot = await voucherService.getPostCodeData(widget.userVoucher!.venuePostcode!);
      ...
      if (userVoucher.venueRestricted != '' && widget.userVoucher!.venueRestricted != userVoucher.venueRestricted) {
        final docData = await firestoreService.getVenue(widget.userVoucher!.venueRestricted!) as DocumentSnapshot;
        showRedAlert('This voucher is can only be redeemed at ' + docData['venueName']);
        return false;
      }
    ...
    try {
      ...
      for (int i = 0; i < discountedVoucherCodes.length; i++) {
        ...
        resp = await callable.call(<String, dynamic>{
          ...
          'venueID': widget.userVoucher!.venueRestricted,
          'selfRedeemed': true,
        });
      }
